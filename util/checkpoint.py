...
def save_checkpoint(epoch, arch, model, optimizer, quan_optimizer,
                    extras=None, is_best=None, name=None, output_dir='.'):
    """Save a pyTorch training checkpoint
    Args:
        epoch: current epoch number
        arch: name of the network architecture/topology
        model: a pyTorch model
        optimizer: a pyTorch optimizer for model paramters
        quan_optimizer: a pyTorch optimizer for quantization parameters
...
def load_checkpoint(model, optimizer, quan_optimizer, chkp_file,
                    model_device=None, strict=False, lean=False):
    ...
    checkpoint = torch.load(chkp_file, map_location=model_device)
    ...
    optimizer.load_state_dict(checkpoint.get('optimizer', None))
    quan_optimizer.load_state_dict(checkpoint.get('quan_optimizer', None))
    ...
    #     for state in quan_optimizer.state.values():
    #         for k, v in state.items():
    #             if torch.is_tensor(v):
    #                 state[k] = v.to(model_device)
    ...
    if lean:
        ...
        return model, 0, None, None, None
    else:
        logger.info("Loaded checkpoint %s model (next epoch %d) from %s", arch, start_epoch, chkp_file)
        return model, start_epoch, optimizer, quan_optimizer, extras
